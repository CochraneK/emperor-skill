# -*- coding: utf-8 -*-
"""把南齐 7 帝登记进主仓 data/modules.json 的 nanbeichao-emperors 模块。
南朝宋8 已建好 module=nanbeichao-emperors / group=nanbeichao-main，本批只追加 7 个新 people 键
（若键已存在则跳过，避免重复计数）。
"""
...
NEW = ["xiaodaochang", "xiaozhe", "xiaozhaoye", "xiaozhaowen", "xiaoluan", "xiaobaojuan", "xiaobaorong"]
...
for sid in NEW:
    ...
    mod = g("module") or "nanbeichao-emperors"
    grp = g("group") or "nanbeichao-main"
    tags_raw = g("tags")
...
print(f"\n[people] +{len(added)}  （原 {len(people) - len(added)} → 现 {len(people)}）")
for x in added:
    print("    " + x)
